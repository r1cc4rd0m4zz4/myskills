...
```bash
node <skill_dir>/web-search/exa_tool.mjs search "notizie geopolitiche oggi" 5
```
...
```bash
# Step 1: Trova l'URL
node <skill_dir>/web-search/exa_tool.mjs search "DeepMind Flamingo paper" 2
# Step 2: Estrai solo la sezione rilevante
node <skill_dir>/web-search/exa_tool.mjs fetch "https://arxiv.org/abs/..." 3500
```
...
```
Usa node <skill_dir>/web-search/exa_tool.mjs per eseguire:
